...
pretty.axis <- function(x, pretty = TRUE, log.scaled = NA, log.zero = 0.1, max.factor = 1, min.factor = 1, include.origin = TRUE, num.labels = 4, max.min.log10.diff = 2){
...
as.power10.expression <- function (x) {
	...
	out <- sapply(x, function(y) {
		...
		if(0 == y){return(expression(bold('0')));}
