...
create.heatmap <- function(x, filename = NULL, clustering.method = 'diana', cluster.dimensions = 'both', rows.distance.method = 'correlation', cols.distance.method = 'correlation', cor.method = 'pearson', row.dendrogram = list(), col.dendrogram = list(), plot.dendrograms = 'both', force.clustering = FALSE, criteria.list = TRUE, covariates = list(), covariates.grid.row = NULL, covariates.grid.col = NULL, covariates.grid.border = NULL, covariates.row.lines = NULL, covariates.col.lines = NULL, covariates.reorder.grid.index = FALSE, covariates.padding = 0.25, covariates.top = list(), covariates.top.grid.row = NULL, covariates.top.grid.col = NULL, covariates.top.grid.border = NULL, covariates.top.row.lines = NULL, covariates.top.col.lines = NULL, covariates.top.reorder.grid.index = FALSE, covariates.top.padding = 0.25, covariate.legends = list(), legend.cex = 1, legend.title.cex = 1, legend.title.just = 'centre', legend.title.fontface = 'bold', legend.border = NULL, legend.border.padding = 1, legend.layout = NULL, legend.between.col = 1, legend.between.row = 1, legend.side = 'left', main = list(label = ''), main.cex = 2, right.size.add = 1, top.size.add = 1, right.dendrogram.size = 2.5, top.dendrogram.size = 2.5, scale.data = FALSE, yaxis.lab = NULL, xaxis.lab = NULL, xaxis.lab.top = NULL, xaxis.cex = NA, yaxis.cex = NA, xlab.cex = 1, ylab.cex = 1, xat = TRUE, xat.top = NULL, yat = TRUE, xaxis.tck = NULL, yaxis.tck = NULL, xaxis.col = 'black', yaxis.col = 'black', col.pos = NULL, row.pos = NULL, cell.text = '', text.font = 1, text.cex = 1, text.col = 'black', colourkey.cex = 3.6, xaxis.rot = 90, yaxis.rot = 0, xlab.label = '' , ylab.label = '', xlab.col = 'black', ylab.col = 'black', axes.lwd = 2, gridline.order = 'h', grid.row = FALSE, grid.col = FALSE, row.lines = seq(0, ncol(x), 1) + 0.5, col.lines = seq(0, nrow(x), 1) + 0.5, colour.scheme = c(), total.colours = 99, colour.centering.value = 0, colour.alpha = 1, fill.colour = 'darkgray', at = NULL, print.colour.key = TRUE, colourkey.labels.at = NULL, colourkey.labels = NULL, top.padding = 0.1, bottom.padding = 0.5, right.padding = 0.5, left.padding = 0.5, x.alternating = 1, shrink = 1, row.colour = 'black', col.colour = 'black', row.lwd = 1, col.lwd = 1, grid.colour = NULL, grid.lwd = NULL, width = 7, height = 7, size.units = 'in', resolution = 1000, enable.warnings = FALSE, xaxis.covariates = NULL, yaxis.covariates = NULL, description = NULL, xaxis.fontface = 'bold', yaxis.fontface = 'bold', symbols = list(borders = NULL, squares = NULL, circles = NULL), same.as.matrix = FALSE, axis.xlab.padding = 0.1, stratified.clusters.rows = NULL, stratified.clusters.cols = NULL) {
	...
	if (exists('dd.row')) {
		...
		if (plot.dendrograms %in% c('both', 'top')) {
			...
			if(length(stratified.clusters.cols) > 0){
				...
				dendrogram.top.grob$children[[1]]$children[[1]]$y0 = dendrogram.top.grob$children[[1]]$children[[1]]$y0[c((1 + length(stratified.clusters.cols)*2):length(dendrogram.top.grob$children[[1]]$children[[1]]$y0))]
				dendrogram.top.grob$children[[1]]$children[[1]]$y1 = dendrogram.top.grob$children[[1]]$children[[1]]$y1[c((1 + length(stratified.clusters.cols)*2):length(dendrogram.top.grob$children[[1]]$children[[1]]$y1))]
				dendrogram.top.grob$children[[1]]$children[[1]]$x0 = dendrogram.top.grob$children[[1]]$children[[1]]$x0[c((1 + length(stratified.clusters.cols)*2):length(dendrogram.top.grob$children[[1]]$children[[1]]$x0))]
				dendrogram.top.grob$children[[1]]$children[[1]]$x1 = dendrogram.top.grob$children[[1]]$children[[1]]$x1[c((1 + length(stratified.clusters.cols)*2):length(dendrogram.top.grob$children[[1]]$children[[1]]$x1))]
				}
			}
	...
	if (exists('dd.col')) {
		...
		if (plot.dendrograms %in% c('both', 'right')) {
			...
			if(length(stratified.clusters.rows) > 0){
				...
				dendrogram.right.grob$children[[1]]$children[[1]]$y0 = dendrogram.right.grob$children[[1]]$children[[1]]$y0[c((1 + length(stratified.clusters.rows)*2):length(dendrogram.right.grob$children[[1]]$children[[1]]$y0))]
				dendrogram.right.grob$children[[1]]$children[[1]]$y1 = dendrogram.right.grob$children[[1]]$children[[1]]$y1[c((1 + length(stratified.clusters.rows)*2):length(dendrogram.right.grob$children[[1]]$children[[1]]$y1))]
				dendrogram.right.grob$children[[1]]$children[[1]]$x0 = dendrogram.right.grob$children[[1]]$children[[1]]$x0[c((1 + length(stratified.clusters.rows)*2):length(dendrogram.right.grob$children[[1]]$children[[1]]$x0))]
				dendrogram.right.grob$children[[1]]$children[[1]]$x1 = dendrogram.right.grob$children[[1]]$children[[1]]$x1[c((1 + length(stratified.clusters.rows)*2):length(dendrogram.right.grob$children[[1]]$children[[1]]$x1))]
				}
			}
	...
	if(colour.alpha == 'automatic'){
		maxX <- max(as.numeric(x));
		minX <- min(as.numeric(x)) ;
		middleX <- (maxX + minX)/2;
		difference = maxX - minX;
		upper.limit.count <- length(as.numeric(x)[as.numeric(x) > maxX - difference*0.2]) + length(as.numeric(x)[as.numeric(x) < minX + difference*0.2]);
		lower.limit.count <- length(as.numeric(x)[as.numeric(x) > middleX - difference*0.2 & as.numeric(x) < middleX + difference*0.2]);
	...
	trellis.object <- lattice::levelplot(
		...
		panel = function(...) {
			...
			panel.levelplot(...);
			panel.text(col.pos, row.pos, cell.text, font = text.font, cex = text.cex, col = text.col);
			positions <- NULL;
			colours <- NULL;
			bordercolours <- NULL;
			sizes <- NULL;
			...
			if ('h' == gridline.order) { # draw horizontal first
				if (grid.row) {
					panel.abline(
						h = row.lines,
						v = 0,
			...
			else if ('v' == gridline.order) { # draw vertical first
				if (grid.col) {
					panel.abline(
						h = 0,
						v = col.lines,
		...
		par.settings = list(
			...
			layout.widths = list(
				...
				key.left = 1,
				key.ylab.padding = if ('' == ylab.label || 0 == length(covariate.legends)) { 0.1 } else { 1.0 },
				ylab = if ('' == ylab.label) { 0.1 } else { 1 },
