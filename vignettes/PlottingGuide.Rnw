...
require("knitr")
opts_chunk$set(fig.path='Examples/', dev='png', warning=FALSE, dpi=100, fig.height=4, fig.width=4, fig.align='center', resize.width='70mm', resize.height='70mm')
@
...
<<scatter3, include=TRUE, fig.cap='Scatterplot with grid lines'>>=
create.scatterplot(
    formula = sample.two ~ sample.one,
    data = scatter.data,
    main = "Gridlines",
...
<<scatter4, include=TRUE, fig.cap='Scatterplot with point labelling'>>=
...
create.scatterplot(
    ...
    # Here the special points are given custom pch and colour    
...
<<scatter5, include=TRUE, fig.cap='Scatterplot with correlation key'>>=
create.scatterplot(
    formula = sample.two ~ sample.one,
    data = scatter.data,
    ...
    # Adding a correlation key using the get.corr.key function
    legend = list(
        inside = list(
            fun = draw.key,
            args = list(
                key = get.corr.key(
                    ...
                    # These are the two vectors of the same length
                    x = scatter.data$sample.one,
                    y = scatter.data$sample.two,
                    
                    # This specifies what to include in the key
                    label.items = c('spearman','kendall','beta1'),
                    
                    # Formatting for the key
                    alpha.background = 0,
                    key.cex = 1
                    )
                ),
                
            # Here are the coordinates for where the place the key
            x = 0.03,
            y = 0.95,
            corner = c(0,1)
            )
...
<<scatter6, include=TRUE, fig.cap='Scatterplot with lines'>>=
create.scatterplot(
    formula = sample.two ~ sample.one,
    data = scatter.data,
    main = "Added lines",
    ...
    # The "r" indicates the addition of a linear regression line
    type = c("p", "g", "r"),
    text.guess.labels = TRUE,
    text.guess.skip.labels = FALSE,
    add.text = TRUE,
...
<<heatmap3, include=TRUE, fig.cap='Heatmap with different clustering'>>=
create.heatmap(
    x = microarray[1:20, 1:20],
    colourkey.cex = 1,
    colourkey.labels.at = seq(2, 12, 1),    
...
<<heatmap4, include=TRUE, fig.cap='Heatmap with stratified clustering'>>=
create.heatmap(
    x = microarray[1:20, 1:20],
    colourkey.cex = 1,
    colourkey.labels.at = seq(2, 12, 1),    
...
<<heatmap5, include=TRUE, fig.cap='Heatmap with covariates'>>=
...
<<heatmap6, include=TRUE, fig.cap='Heatmap with grid lines'>>=
create.heatmap(
    x = microarray[1:20, 1:20],
    colourkey.cex = 1,
    colourkey.labels.at = seq(2, 12, 1),    
...
<<heatmap7, include=TRUE, fig.cap='Heatmap with discrete colours'>>=
create.heatmap(
    x = microarray[1:20, 1:20],
    colourkey.cex = 1,
    colourkey.labels.at = seq(2, 12, 1),    
...
<<heatmap8, include=TRUE, fig.cap='Heatmap with symbols'>>=
create.heatmap(
    x = microarray[1:20, 1:20],
    colourkey.cex = 1,
    colourkey.labels.at = seq(2, 12, 1),    
   ...
   # Adding symbols using one method
...
<<dotmap1, include=TRUE, fig.cap='Simple dotmap'>>=
...
<<hexbin1, include=TRUE, fig.cap='Simple hexbinplot'>>=
...
# Printing the barplot to show what it looks like
...
<<multiplotex1_final, include=TRUE, fig.cap='Example 1 multiplot', fig.height = 6, fig.width = 6, resize.height ='80mm', resize.width ='80mm'>>=
 create.multiplot(
         ...
         # The spacing between the plots is set
         yspacing = c(-1, -1, -1),
         xaxis.lab = NULL,
...
importance.barplot <- create.barplot(
     ...
     # Note that if real data was used, we would have to ensure that the corresponding 
...
legend1 <- legend.grob(
    ...
    # This adds a black box around the legend
...
create.multiplot(
    ...
    yspacing = c(0.5, 0.5, 0.5),

    # Providing space for the legend
    bottom.padding = 15,
    ...
    legend = list(
    	...
    	# Placing legend inside plotting space
        inside = list(
            fun = legend1,
            x = 0.5,
            y = -0.1,
        ...
        # Placing legend to the right-hand side
        right = list(
        	fun = legend2
        	)
        )
